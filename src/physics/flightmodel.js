...
export class FlightModel_PfGD extends Component {
    ...
    _calcMassProperties() {
        this.elements.push({
            ...
            LocalInertia: new THREE.Vector3(13.92, 10.5, 24.0),
            Incidence: -3.5,
            Dihedral: 0.0,
            Area: 31.2,
            Flap: 0,
        ...
        this.elements.push({
            ...
            LocalInertia: new THREE.Vector3(66.3, 861.9, 861.9),
            Incidence: 0.0,
            Dihedral: 0.0,
            Area: 84.0,
            Flap: 0,
        ...
        for (const element of this.elements) {
            Ixx +=
                element.LocalInertia.x +
                element.Mass *
                    (element.CGCoords.y * element.CGCoords.y +
                        element.CGCoords.z * element.CGCoords.z);
            Iyy +=
                element.LocalInertia.y +
                element.Mass *
                    (element.CGCoords.z * element.CGCoords.z +
                        element.CGCoords.x * element.CGCoords.x);
            Izz +=
                element.LocalInertia.z +
                element.Mass *
                    (element.CGCoords.x * element.CGCoords.x +
                        element.CGCoords.y * element.CGCoords.y);
        ...
        this.Inertia.set(Ixx, -Ixy, -Ixz, -Ixy, Iyy, -Iyz, -Ixz, -Iyz, Izz);
            ...
            if (a[i] <= angle && a[i + 1] > angle) {
                switch (flaps) {
                    case 0: // flaps not deflected
                        cl =
                            clf0[i] -
                            ((a[i] - angle) * (clf0[i] - clf0[i + 1])) /
                                (a[i] - a[i + 1]);
                        break;
                    case -1: // flaps down
                        cl =
                            clfd[i] -
                            ((a[i] - angle) * (clfd[i] - clfd[i + 1])) /
                                (a[i] - a[i + 1]);
                        break;
                    case 1: // flaps up
                        cl =
                            clfu[i] -
                            ((a[i] - angle) * (clfu[i] - clfu[i + 1])) /
                                (a[i] - a[i + 1]);
                        break;
                }
                break;
        ...
        for (let i = 0; i < 8; i++) {
            if (a[i] <= angle && a[i + 1] > angle) {
                ...
                if (flaps < 0) {
                    ...
                    let flap_down =
                        cdfd[i] -
                        ((a[i] - angle) * (cdfd[i] - cdfd[i + 1])) /
                            (a[i] - a[i + 1]);
                    cd = Math.lerp(no_flaps, flap_down, t);
                } else {
                    let flap_up =
                        cdfu[i] -
                        ((a[i] - angle) * (cdfu[i] - cdfu[i + 1])) /
                            (a[i] - a[i + 1]);
    ...
    _rudderLiftCoeff(angle) {
        ...
        for (let i = 0; i < 6; i++) {
            if (a[i] <= aa && a[i + 1] > aa) {
                cl =
                    clf0[i] -
                    ((a[i] - aa) * (clf0[i] - clf0[i + 1])) / (a[i] - a[i + 1]);
                if (angle < 0) cl = -cl;
                break;
            }
        }
    ...
    _rudderDragCoeff(angle) {
        ...
        for (let i = 0; i < 6; i++) {
            if (a[i] <= aa && a[i + 1] > aa) {
                cd =
                    cdf0[i] -
                    ((a[i] - aa) * (cdf0[i] - cdf0[i + 1])) / (a[i] - a[i + 1]);
                break;
            }
        }
        return cd;
